...
class ViewOrder extends Component {
  ...
  getOrderData = async () => {
    ...
    this.setState({ orderTotalPrice: this.calculateTotalPrice() });
  };
  ...
  render() {
    ...
    return (
      <>
        ...
        <Container data-cy="order-list">{orderItems}</Container>
        <p data-cy="total-price">Total price: {this.state.totalPrice}Kr</p>
        <Button
          onClick={() => {
            this.finalizeOrder();
          }}
          data-cy="finalize-order-button"
        >
          Confirm Order
        </Button>
        {this.state.pickUpTime && (
          <>
            ...
            <p data-cy="order-confirmation-ready-time">
              Your order will be ready at {this.state.pickUpTime}
            </p>
          </>
        )}
      </>
